            ...
            <div className="hidden lg:flex items-center gap-2">
              {categories.map((category) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm capitalize fmo-transition bg-popover ${
                    selectedCategory === category
                      ? "bg-primary text-primary-foreground"
                      : "bg-card text-foreground/70 hover:text-foreground fmo-shadow"
                  }`}
                >
                  {category}
                </button>
                ...
                <div className="space-y-3">
                  {categories.map((category) => (
                    <button
                      ...
                      className={`w-full px-6 py-4 rounded-2xl text-left capitalize fmo-transition ${
                        selectedCategory === category
                          ? "bg-primary text-primary-foreground"
                          : "bg-card text-foreground fmo-shadow"
                      }`}
                    >
                      {category}
                    </button>
...
function ProductCard({ 
  ...
  return (
    ...
    >
      <div className="bg-card rounded-3xl overflow-hidden fmo-shadow fmo-transition group-hover:scale-[1.02]">
        {/* Image */}
        <div className="relative aspect-square bg-muted overflow-hidden">
          ...
          <Image
            ...
            alt={product.name}
            fill
            className={`object-cover fmo-transition group-hover:scale-105 transition-opacity duration-500 ${
          ...
          <button
            type="button"
            className="absolute bottom-4 right-4 w-12 h-12 rounded-full bg-background/90 backdrop-blur-sm flex items-center justify-center opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 fmo-transition fmo-shadow"
            onClick={(e) => {
              e.preventDefault()
            }}
